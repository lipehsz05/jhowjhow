...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        switch ($periodo) {
            ...
            case 'yearly':
                ...
                break;
            case 'custom':
                $dataInicio = $request->input('start_date', $request->input('data_inicio'))
                    ? Carbon::parse($request->input('start_date', $request->input('data_inicio')))->startOfDay()
                    : Carbon::now()->startOfMonth();
                $dataFim = $request->input('end_date', $request->input('data_fim'))
                    ? Carbon::parse($request->input('end_date', $request->input('data_fim')))->endOfDay()
                    : Carbon::now()->endOfDay();
                break;
        }
    ...
    {
        // Diário e ontem: agregação por hora; demais: por dia
        if ($periodo === 'daily' || $periodo === 'yesterday') {
            $format = '%Y-%m-%d %H:00';
        } else {
            $format = '%Y-%m-%d';
        }
